...
{
    public partial class DayInstance : Form
    {
        public DayInstance()
        {
            InitializeComponent();
        ...
        private void DayInstance_Load(object sender, EventArgs e)
        {
            cmbRange.Text = "个人";
        ...
        private void cmbRange_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if(cmbRange.Text=="家庭")
            {
                ...
                {
                    string connectstring = "provider=Microsoft.jet.OLEDB.4.0;Data Source=accountsystem.mdb;Jet OleDb:Database Password=lxw618";
                    string sqlcommand = "select [ID] as 数据号,[username] as 用户名,[date] as 收支日期,[money] as 金额,[item] as 收支项目,[type] as 收支类型,[memo] as 备注 from [Money] where [date]=#" + dtStr + "# order by [ID]";
                    OleDbConnection aConnection = new OleDbConnection(connectstring);
                    aConnection.Open();
                ...
                try
                {
                    ...
                    OleDbConnection aConnection = new OleDbConnection(connectstring);
                    aConnection.Open();
                    string sqlcommandC = "select sum([money]) from [Money] where [date]=#" + dtStr + "# and [type]='收入'";
                    ...
                    string sqlcommandD = "select sum([money]) from [Money] where [date]=#" + dtStr + "# and [type]='支出'";
                    ...
                    lbGet.Text = "当日共收入" + strIncome + "元，支出" + strOutcome + "元，盈余" + strGet + "元。";
                    aConnection.Close();
                }
                catch
                {
            ...
            else
            {
                ...
                try
                {
                    ...
                    OleDbConnection aConnection = new OleDbConnection(connectstring);
                    aConnection.Open();
                    string sqlcommandC = "select sum([money]) from [Money] where [username]='" + cmbUsername.Text + "' and [date]=#" + dtStr+ "# and [type]='收入'";
                    OleDbCommand cCommand = new OleDbCommand(sqlcommandC, aConnection);
                    OleDbDataReader creader = cCommand.ExecuteReader();
                    creader.Read();
                    string strIncome, strOutcome;
                    ...
                    string sqlcommandD = "select sum([money]) from [Money] where [username]='" + cmbUsername.Text + "' and [date]=#" + dtStr + "# and [type]='支出'";
                    OleDbCommand dCommand = new OleDbCommand(sqlcommandD, aConnection);
                    OleDbDataReader dreader = dCommand.ExecuteReader();
                    dreader.Read();
                    strOutcome = dreader.GetValue(0).ToString();
